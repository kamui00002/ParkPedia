      ...
      <div className="bg-white rounded-lg shadow-md p-6">
        <form onSubmit={handleSubmit}>
          ...
          <div className="space-y-6">
            <div>
              <label htmlFor="park-name" className="block text-base font-semibold text-gray-800 mb-1">公園名 <span className="text-red-500">*</span></label>
              <input
                type="text"
                id="park-name"
                value={name}
                onChange={(e) => setName(e.target.value)}
                className="w-full px-3 py-2 bg-white border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500 focus:outline-none text-sm"
                placeholder="例: ひまわり中央公園"
              />
            </div>

            <div>
              <label htmlFor="park-address" className="block text-base font-semibold text-gray-800 mb-1">住所 <span className="text-red-500">*</span></label>
              <input
                type="text"
                id="park-address"
                value={address}
            ...
            <div>
              <label className="block text-base font-semibold text-gray-800 mb-1">写真 ({photos.length}/{MAX_PHOTOS}) <span className="text-red-500">*</span></label>
              <p className="text-sm text-gray-500 mb-3">最初の写真がメイン画像になります。</p>
              <div className="flex items-center gap-3">
                <label htmlFor="photo-upload" className="cursor-pointer bg-green-500 text-white px-4 py-2 rounded-lg text-sm font-semibold hover:bg-green-600 transition-colors">
                  ファイル選択
                </label>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                      {category.options.map(option => (
                        <label key={option} className="flex items-center cursor-pointer">
                          ...
                          />
                          <span className="h-4 w-4 rounded border-2 border-gray-400 bg-white peer-checked:border-green-600 peer-checked:bg-white flex items-center justify-center transition-colors mr-2">
          ...
          <div className="mt-8 pt-4 border-t flex justify-end space-x-3">
            <button type="button" onClick={onCancel} className="px-6 py-2 bg-white border border-gray-300 rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-50">
              キャンセル
            </button>
            <button type="submit" className="px-6 py-2 bg-green-500 text-white rounded-lg text-sm font-medium hover:bg-green-600 transition-colors">
              この公園を登録する
            </button>
          </div>
        </form>
