...
const RecommendedCard: React.FC<{ park: Park; onSelectPark: (park: Park) => void }> = ({ park, onSelectPark }) => {
  ...
  return (
    <div
      ...
      className="flex-shrink-0 w-56 bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden cursor-pointer transform hover:shadow-md transition-shadow duration-200"
    >
      <img src={park.mainImage} alt={park.name} className="w-full h-40 object-cover" />
      <div className="p-3">
        <h4 className="text-base font-bold text-gray-800 truncate">{park.name}</h4>
...
const RecommendedParks: React.FC<RecommendedParksProps> = ({ parks, onSelectPark }) => {
  ...
  return (
    <div>
      <h2 className="text-xl font-bold text-gray-800 mb-3">あなたへのおすすめ</h2>
