...
const RecommendedCard: React.FC<{ park: Park; onSelectPark: (park: Park) => void }> = ({ park, onSelectPark }) => {
  ...
  return (
    <div
      ...
      className="flex-shrink-0 w-56 bg-white rounded-lg shadow-md overflow-hidden cursor-pointer transform hover:shadow-lg transition-shadow duration-200"
    >
      <img src={park.mainImage} alt={park.name} className="w-full h-32 object-cover" />
      <div className="p-3">
        <h4 className="text-base font-bold text-gray-800 truncate mb-1">{park.name}</h4>
        ...
        <p className="text-xs text-gray-500 truncate">{park.address}</p>
      </div>
    </div>
  );
};
