...
const InteractiveStar: React.FC<{ filled: boolean; onMouseEnter: () => void; onClick: () => void; }> = ({ filled, onMouseEnter, onClick }) => (
    <svg 
        ...
        onClick={onClick}
        className={`w-7 h-7 cursor-pointer ${filled ? 'text-yellow-400' : 'text-gray-300'}`} 
      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto m-4">
        <form onSubmit={handleSubmit}>
          <div className="p-6">
            ...
            <div className="mt-6">
              ...
              <div className="relative">
                <textarea
                  ...
                  className="w-full p-3 bg-white border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500 focus:outline-none text-sm"
                  value={comment}
            ...
            <div className="mt-6">
               <label className="block text-sm font-medium text-gray-700 mb-2">写真を追加 ({photos.length}/{MAX_PHOTOS})</label>
                <div className="p-3 bg-yellow-50 border-l-4 border-yellow-400 text-yellow-800 text-sm mb-3 rounded">
                    <p className="font-bold mb-1">投稿ルールのお願い</p>
                    <p className="text-xs">トラブル防止のため、人物の顔が特定できる写真の投稿は避けてください。</p>
                </div>
          ...
          <div className="bg-gray-50 px-6 py-4 flex justify-end space-x-3 border-t">
            <button type="button" onClick={onClose} className="px-6 py-2 bg-white border border-gray-300 rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-50">
              キャンセル
            </button>
