      ...
      <div className="bg-white rounded-lg shadow-md overflow-hidden">
        ...
        <div className="px-4 sm:px-6 py-3 bg-gray-50 border-y">
            <div className="flex items-center space-x-2 overflow-x-auto pb-2">
                <button onClick={() => setActivePhotoCategory('all')} className={`px-4 py-1.5 text-sm font-semibold rounded-full transition-colors whitespace-nowrap ${activePhotoCategory === 'all' ? 'bg-green-500 text-white' : 'bg-white text-gray-700 hover:bg-green-50'}`}>全て</button>
                {PHOTO_CATEGORIES.map(cat => (
                    <button key={cat.id} onClick={() => setActivePhotoCategory(cat.id)} className={`px-4 py-1.5 text-sm font-semibold rounded-full transition-colors whitespace-nowrap ${activePhotoCategory === cat.id ? 'bg-green-500 text-white' : 'bg-white text-gray-700 hover:bg-green-50'}`}>{cat.name}</button>
                ))}
            </div>
        </div>
        ...
        <div className="flex overflow-x-auto gap-1 p-1">
          {filteredImages.map((img, index) => (
            <img key={index} src={img.url} alt={`${park.name} photo ${index + 1}`} className="h-48 object-cover flex-shrink-0" style={{ minWidth: '200px' }} />
          ))}
        </div>
            ...
            <div className="relative">
              ...
              <div className="relative rounded-lg overflow-hidden border" style={{ height: '300px' }}>
                ...
                <button 
                  ...
                  className="absolute bottom-3 right-3 bg-green-500 text-white font-semibold py-2 px-4 rounded-full hover:bg-green-600 transition-colors flex items-center shadow-lg"
                >
