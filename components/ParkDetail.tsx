      ...
      <div className="bg-white rounded-lg shadow-xl overflow-hidden">
        ...
        <div className="px-4 sm:px-6 py-3 bg-gray-50 border-y">
            <div className="flex items-center space-x-2 overflow-x-auto">
                <button onClick={() => setActivePhotoCategory('all')} className={`px-4 py-2 text-sm font-semibold rounded-lg transition-colors whitespace-nowrap ${activePhotoCategory === 'all' ? 'bg-green-600 text-white' : 'bg-white text-gray-700 hover:bg-gray-100'}`}>全て</button>
                {PHOTO_CATEGORIES.map(cat => (
                    <button key={cat.id} onClick={() => setActivePhotoCategory(cat.id)} className={`px-4 py-2 text-sm font-semibold rounded-lg transition-colors whitespace-nowrap ${activePhotoCategory === cat.id ? 'bg-green-600 text-white' : 'bg-white text-gray-700 hover:bg-gray-100'}`}>{cat.name}</button>
                ))}
            </div>
        </div>

        <div className="flex space-x-2 overflow-x-auto px-4 sm:px-6 py-4">
          {filteredImages.map((img, index) => (
            <img key={index} src={img.url} alt={`${park.name} photo ${index + 1}`} className="flex-shrink-0 w-32 h-32 object-cover rounded-lg" />
          ))}
        </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="relative">
              ...
              <button 
                ...
                className="absolute bottom-4 right-4 bg-green-600 text-white font-bold py-2 px-4 rounded-lg hover:bg-green-700 transition-colors flex items-center shadow-lg"
              >
